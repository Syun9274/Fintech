...
Table Accounts {
  ...
  user_id BIGINT [not null, ref: > Users.id] // 사용자 ID (외래 키)
...
Table Transactions {
  ...
  sender_account_id BIGINT [ref: > Accounts.id] // 송금자 계좌 ID (외래 키)
  receiver_account_id BIGINT [ref: > Accounts.id] // 수신자 계좌 ID (외래 키)
...
Table RequestLogs {
  ...
  user_id BIGINT [not null, ref: > Users.id] // 요청한 사용자 ID (외래 키)
  account_id BIGINT [ref: > Accounts.id] // 요청 대상 계좌 ID (외래 키, NULL 가능)
